...
function getWarningMessage(warningType, warning) {
  // Create concise, user-friendly messages
  const warningMessages = {
    'WTCSGNL': 'Typhoon Signal',
    'WRAIN': 'Rainstorm Warning',
    'WL': 'Landslip Warning',
    'WFNTSA': 'Flooding Alert',
  ...
  const baseMessage = warningMessages[warningType] || warning.name || warningType;
  
  // Add severity level if available
  if (warning.type && warning.type !== 'NIL') {
    return `${baseMessage} (${warning.type})`;
  }
