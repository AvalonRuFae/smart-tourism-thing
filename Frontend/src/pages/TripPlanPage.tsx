...
const AttractionTab = styled.button<{ isActive: boolean }>`
	background: ${(props) =>
		props.isActive ? theme.colors.primary : theme.colors.surface};
	...
	&:hover {
		background: ${(props) =>
			props.isActive ? theme.colors.primaryDark : theme.colors.surfaceDark};
...
const RouteStep = styled.div<{ isActive: boolean }>`
	background: ${(props) =>
		props.isActive ? theme.colors.primaryLight : theme.colors.background};
	border-radius: ${theme.borderRadius.md};
	padding: ${theme.spacing.md};
	min-width: 250px;
	flex-shrink: 0;
...
const StepDetails = styled.div<{ isActive: boolean }>`
	color: ${(props) =>
		props.isActive ? "rgba(255,255,255,0.9)" : theme.colors.textSecondary};
	font-size: 0.9rem;
	line-height: 1.4;
`;
...
const TripPlanPage: React.FC = () => {
	...
	const getAttractionsData = () => {
		if (
			tripPlanData &&
			(tripPlanData.plannedAttractions || tripPlanData.recommendations)
		) {
			const attractions = (
				tripPlanData.plannedAttractions ||
				tripPlanData.recommendations ||
				[]
				...
				if (tripPlanData.plannedAttractions) {
					...
					if (
						typeof item.plannedStartTime === "string" &&
						item.plannedStartTime.match(/^\d{1,2}:\d{2}$/)
					) {
						...
						startTime = new Date(item.plannedStartTime).toLocaleTimeString(
							"en-US",
							{
								hour: "2-digit",
								minute: "2-digit",
					...
					console.log(
						"   📅 Original suggestedTime:",
						item.suggestedTime,
						typeof item.suggestedTime
					);
				...
				return {
					...
					location: attraction.location || {
						lat: 22.3193 + index * 0.01,
						lng: 114.1694 + index * 0.01,
					},
					timing: {
						startTime,
						duration,
					...
					transport: {
						fromPrevious:
							index === 0 ? "Start of journey" : "From previous location",
					...
					cost: item.estimatedCost
						? `HK$${item.estimatedCost}`
						: attraction.priceRange || "Varies",
					difficulty: "Easy",
					description: attraction.description || "AI recommended attraction",
					highlights: [
						item.reason ||
							attraction.aiRecommendation?.reason ||
							"Recommended by AI",
					],
	...
	const selectedAttraction = currentTripData.attractions[
		selectedIndex
	] as Attraction;
          ...
          <div style="padding: 12px; max-width: 250px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 0.8rem;">
              ...
              <div><strong>Duration:</strong> ${
								attraction.timing.duration
							} min</div>
              <div><strong>Cost:</strong> ${attraction.cost}</div>
            </div>
            ${
							isSelected
								? '<p style="margin: 8px 0 0 0; color: #D32F2F; font-weight: bold; font-size: 0.8rem;">Currently Selected</p>'
								: ""
						}
          </div>
        `,
	...
	const updateTransportInfo = (
		...
		if (route && route.legs[0]) {
			...
			let cost = "HK$10-30";
			let instructions =
				leg.steps[0]?.instructions || "Follow the directions on map";
			...
			const hasWalking = leg.steps.some(
		...
		<PageContainer>
			<Header>
				...
				<AttractionSelector>
					{currentTripData.attractions.map((attraction: any, index: number) => (
						...
						>
							{index + 1}. {(attraction as Attraction).name}
						</AttractionTab>
					))}
				</AttractionSelector>
			</Header>
			...
			<MainContent>
						...
						>
							<div style={{ textAlign: "center", color: "#666" }}>
								...
								<p style={{ fontSize: "14px", marginTop: "10px" }}>
									Add REACT_APP_GOOGLE_MAPS_API_KEY to enable maps
								</p>
							</div>
						</div>
					)}
				...
				<AttractionSlider>
					<SliderHeader>
						<SliderTitle>
							{selectedIndex + 1}. {selectedAttraction.name}
						</SliderTitle>
					...
					<SliderContent>
						...
						<Description>{selectedAttraction.description}</Description>
							...
							{isLoadingRoute ? (
									...
									<TransportGrid>
										<InfoItem>
											...
											<InfoValue>
												{selectedIndex === 0
													? "Hong Kong Central"
													: currentTripData.attractions[selectedIndex - 1].name}
											</InfoValue>
										</InfoItem>
										<InfoItem>
								...
								<TransportGrid>
									<InfoItem>
										<InfoLabel>From</InfoLabel>
										<InfoValue>
											{selectedAttraction.transport.fromPrevious}
										</InfoValue>
									</InfoItem>
									<InfoItem>
										<InfoLabel>Method</InfoLabel>
									...
									<InfoItem>
										<InfoLabel>Duration</InfoLabel>
										<InfoValue>
											{selectedAttraction.transport.duration} min
										</InfoValue>
									</InfoItem>
									<InfoItem>
										<InfoLabel>Cost</InfoLabel>
					...
					<RouteSteps>
						{currentTripData.attractions.map(
							(attraction: any, index: number) => (
								...
								>
									<StepNumber isActive={index === selectedIndex}>
										{index + 1}
									</StepNumber>
									<StepContent>
										...
										<StepDetails isActive={index === selectedIndex}>
											<strong>
												{(attraction as Attraction).timing.startTime}
											</strong>{" "}
											• {(attraction as Attraction).timing.duration} min
											<br />
											{index === selectedIndex && routeInfo
												? `${routeInfo.method} • ${routeInfo.cost} (Google Maps)`
												: `${(attraction as Attraction).transport.method} • ${
														(attraction as Attraction).transport.cost
												  }`}
										</StepDetails>
									</StepContent>
								</RouteStep>
