...
import { UserInputProps, UserPreferences, AttractionCategory } from '../../types';
...
const TransportOption = styled.div<{ selected: boolean }>`
  ...
  gap: ${theme.spacing.sm};
  padding: ${theme.spacing.md};
  border: 2px solid ${({ selected }) => selected ? theme.colors.primary : theme.colors.surfaceDark};
  border-radius: ${theme.borderRadius.md};
  cursor: pointer;
  transition: all 0.2s ease;
  background: ${({ selected }) => selected ? `${theme.colors.primary}10` : theme.colors.surface};
  ...
  svg {
    ...
    color: ${({ selected }) => selected ? theme.colors.primary : theme.colors.textSecondary};
  }
`;
...
export const UserInput: React.FC<UserInputProps> = ({
  ...
  const handleActivityChange = (category: AttractionCategory, checked: boolean) => {
    setPreferences(prev => ({
      ...prev,
  ...
  const transportOptions = [
    ...
    { key: 'public_transport', icon: <TransitIcon />, label: 'Public Transport' },
  ...
  return (
    <InputContainer>
      ...
      <Flex align="center" gap={theme.spacing.md}>
        <div style={{ flex: 1, height: '1px', background: theme.colors.surfaceDark }}></div>
      ...
      <div>
            ...
            {activityOptions.map(option => (
              <CheckboxItem key={option.key}>
                <input
                  ...
                  onChange={(e) => handleActivityChange(option.key, e.target.checked)}
                />
                {option.label}
              </CheckboxItem>
            ))}
        ...
        <FormGroup style={{ marginTop: theme.spacing.lg }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: theme.spacing.sm }}>
        ...
        >
          {onTripPlan ? (
            ...
            <Button 
              ...
              size="lg"
            >
              {isLoading ? <LoadingSpinner /> : preferencesSubmitted ? '✓ Preferences Applied' : 'Analyze My Preferences'}
            </Button>
          )}
        </Flex>
        ...
        {preferencesSubmitted && (
          <ConfirmationMessage>
            <span>✅</span>
            Your preferences have been applied! Check the map and attractions for personalized recommendations.
          </ConfirmationMessage>
        )}
      </div>
    </InputContainer>
