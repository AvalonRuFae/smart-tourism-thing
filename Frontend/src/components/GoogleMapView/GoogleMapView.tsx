...
const ToggleButton = styled(Button)<{ active?: boolean }>`
	...
	border: 1px solid
		${(props) =>
			props.active ? theme.colors.primary : theme.colors.surfaceDark};
	font-size: 0.875rem;
	padding: ${theme.spacing.sm} ${theme.spacing.md};

	&:hover {
		background: ${(props) =>
			props.active ? theme.colors.primaryDark : theme.colors.surface};
	}
`;
...
const SAMPLE_HK_ATTRACTIONS = [
	{
		...
		name: "The Peak",
		description:
			"Iconic mountain offering panoramic views of Hong Kong skyline",
	...
	},
	{
		...
		description:
			"Waterfront promenade with stunning harbor views and cultural attractions",
	...
	},
	{
		...
		name: "Ocean Park",
		description:
			"Marine life theme park with thrilling rides and animal exhibits",
...
const GoogleMapView: React.FC<GoogleMapViewProps> = ({
	...
	const [internalSelectedAttraction, setInternalSelectedAttraction] =
	...
	const handleAttractionClick = useCallback(
		async (attraction: TouristAttraction) => {
			try {
	...
	const optimizeMultiLocationRoute = useCallback(
		async (locations: Array<{ lat: number; lng: number; name: string }>) => {
			if (
	...
	const showSimpleDemoRoute = useCallback(async () => {
		...
		const origin =
			attractions.length > 0 ? attractions[0] : SAMPLE_HK_ATTRACTIONS[0]; // The Peak
		const destination =
			attractions.length > 0
				? attractions[Math.min(3, attractions.length - 1)]
				: SAMPLE_HK_ATTRACTIONS[3]; // Ocean Park
